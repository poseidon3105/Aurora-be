...
export class ChecklistsService {
  ...
  // ───────────────────────────
  private async validateChecklistProjectActive(checklist: {
    projectId: number;
    deletedAt: Date | null;
  }) {
    ...
    if (project.status !== ProjectStatus.ACTIVE) {
      throw new BadRequestException('Project must be ACTIVE to modify checklists');
    }
  ...
  async update(checklistId: number, dto: UpdateChecklistDto, userId: number) {
    const checklist = await this.findChecklistOrThrow(checklistId);
    const project = await this.validateChecklistProjectActive(checklist);

    // Authorization: PROJECT_MANAGER or Checklist Creator (who is still an active member)
    ...
    if (!isMember || (!isManager && !isCreator)) {
      throw new ForbiddenException(
        'Only an active project manager or the active checklist creator can update this checklist',
      );
    }
  ...
  async changeStatus(checklistId: number, dto: ChangeChecklistStatusDto, userId: number) {
    const checklist = await this.findChecklistOrThrow(checklistId);
    await this.validateChecklistProjectActive(checklist);

    // Authorization: Must be an active project member
    const isMember = await this.isProjectMember(checklist.projectId, userId);
    if (!isMember) {
      throw new ForbiddenException('You are not a member of this project');
    }
  ...
  async remove(checklistId: number, userId: number) {
    ...
    if (!isMember || (!isManager && !isCreator)) {
      throw new ForbiddenException(
        'Only an active project manager or the active checklist creator can delete this checklist',
      );
    }
    // A checklist can only be deleted after every non-deleted task is DONE.
    const doneStatus = await this.prisma.taskStatus.findFirst({
      where: { name: 'DONE' },
    });
    const incompleteTask = await this.prisma.checklistItem.findFirst({
      where: {
        checklistId,
        deletedAt: null,
        ...(doneStatus ? { statusId: { not: doneStatus.id } } : {}),
      },
    });
    if (incompleteTask) {
